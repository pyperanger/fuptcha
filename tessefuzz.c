...
_alphaword(char* T)
{
  ...
  while (T[c] != '\0') {
    ret[c][0] = T[c];
    if ((((int)ret[c][0] >= 65) && ((int)ret[c][0] <= 90)) ||
        (((int)ret[c][0] >= 97) && ((int)ret[c][0] <= 122)))
      ret[c][1] =
        (int)ret[c][0] < 91 ? (int)ret[c][0] + 32 : (int)ret[c][0] - 32;
    else
      ret[c][1] = ret[c][0];
    ++c;
  }
...
int
scorepoint(char* text, WORD capinfo, int lang)
{
...
shoothesheriff(WORD capinfo)
{
  ...
  while (langs[c] != NULL) {
    ...
    printf("\r\x1B[33m~ [  %d/%d  ] \t%s\t\t\t%d points\t-> %s", c, langlen - 1,
           langs[c], rank[c][1], textrecon);
    fflush(stdout);
...
{
  if (argc < 3)
    die("TessFuZz-> FiND THe BEsT lANg to your captcha "
        "challage\nSearch level: "
        "1 LOW , 2 MEDIUN, 3 INSANE\nUsage: ./tessefuzz foto.jpg "
        "\"AvZd\" "
        "1\n\n");
  ...
  puts("\x1B[36m[Info]\x1B[37m Welcome Tesseract Fuzz, lets try find the "
       "best "
       "language configuration to bypass and fuck some systemadmin life");

  WORD CAPTCHA = {.text = argv[2],
                  .filename = argv[1],
                  .words = _alphaword(argv[2]),
                  .lentext = strlen(argv[2]),
                  .fpoint = 100 / CAPTCHA.lentext,
                  .mpoint = CAPTCHA.fpoint / CAPTCHA.lentext,
                  .level = 1 };
